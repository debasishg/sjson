...
class SerializerSpec extends FunSpec with MustMatchers {
  ...
  describe("String serialization") {
    it("should give an instance of String") {
      serializer.in[String](
        serializer.out("debasish ghosh")) must equal("debasish ghosh")
    }
    it("should give a null instance of String") {
      serializer.in[String](
        serializer.out(null)) must equal(null)
      serializer.in[String](
        serializer.out("null")) must equal("null")
    }
  }
  ...
  describe("Number serialization") {
    it("should give an instance of BigDecimal") {
      serializer.in[BigDecimal](
        serializer.out(BigDecimal(20))) must equal(BigDecimal(20))

      serializer.in[BigDecimal](
        serializer.out(BigDecimal(20))) must equal(20)
    }
  }
  ...
  describe("Simple bean serialization") {
    ...
    it("should give an instance of Address") {
      serializer.in[Address](
        serializer.out(addr)) must equal(addr)
    }
  }
  ...
  describe("Bean with embedded object serialization") {
    ...
    it("should give an instance of Contact") {
      c must equal(serializer.in[Contact](co))
    }
  }
  ...
  describe("Inherited bean serialization") {
    ...
    it("should give an instance of the derived class InternationalAddress") {
      ...
      addr.country must equal(ad.country)
      addr.city must equal(ad.city)
      addr.street must equal(ad.street)
      addr.zip must equal(ad.zip)
    }
    it("should serialize a slice and give an instance of Address") {
      val a3 = serializer.in[Address](
        serializer.out(addr))
      a3.city must equal(addr.city)
      a3.street must equal(addr.street)
      a3.zip must equal(addr.zip)

      new Address(addr.street, addr.city, addr.zip) must equal(
        serializer.in[Address](
          serializer.out(addr)))
    }
  }
  ...
  describe("Serialization with ignore properties") {
    it("should ignore issn field") {
      ...
      serializer.in[Journal](serializer.out(j)).issn must equal(null)
    }
  }
  ...
  describe("Serialization of Boolean") {
    it("should serialize properly") {
      ...
      serializer.in[Foo](serializer.out(b)) must equal(b)
    }
  }
  ...
  describe("Serialization of vals") {
    it("should serialize properly") {
      ...
      serializer.in[Bar](serializer.out(b)) must equal(b)
    }
  }
  ...
  describe("Serialization of timezones") {
    it("should serialize properly") {
      ...
      serializer.in[TimeZoneBean](serializer.out(t)) must equal(t)
    }
  }
  ...
  describe("Serialization of enumerations") {
    it("should serialize properly") {
      ...
      b.start must equal(o.start)
      b.shape must equal(o.shape)
      b.month must equal(o.month)
      b.work must equal(o.work)
      b.months must equal(o.months)
    }
  }
  ...
  describe("Serialization of date") {
    it("should serialize properly") {
      ...
      serializer.in[SecurityTrade](serializer.out(t)) must equal(t)
    }
  }
  ...
  describe("Serialization of tuple") {
    ...
    it("should serialize an object with Tuple2") {
      ...
      new String(json) must equal("""{"id":"id","value":{"hello":34}}""")
      val f = serializer.in[MyMessage](json)
      f must equal(message)
    }
  }
  ...
  describe("Serialization with classloader API") {
    it("should serialize properly and de-serialize") {
      ...
      MySJSON.in(s, "java.lang.String") must equal("debasish")
      ...
      insh.store must equal(shop.store)
      insh.item must equal(shop.item)
      insh.price must equal(shop.price)
    }
    it("should serialize an object with Tuple2") {
      val message = MyMessage("id", ("hello", 34))
      val json = new String(MySJSON.out(message))
      json must equal("""{"id":"id","value":{"hello":34}}""")
    }
  }
  ...
  describe("Bean with no default constructor using Objenesis") {
    it("should serialize properly and de-serialize") {
      ...
      MyObjenesisSJSON.in(s, "java.lang.String") must equal("joewalnes")
  ...
  describe("Bean serialization with class loading") {
    ...
    it("should give an instance of Address") {
      MySJSON.in(
        MySJSON.out(addr), "sjson.json.TestBeans$Address") must equal(addr)
    }
  }
  ...
  describe("Testing Array serialization") {
    it ("should do proper Array serialization") {
      ...
      new String(out) must equal("""{"addresses":["a","b","c"],"id":100,"name":"debasish"}""")
      val in = serializer.in[ArrayTest](out)
      in.addresses must equal(Array("a", "b", "c"))
    }
  }
  ...
  describe("Testing object array serialization") {
    ...
    it("should do proper serialization of object arrays") {
      ...
      in.addresses must equal(Array(a1, a2, a3))
    }
  }
  ...
  describe("Testing for an object that contains an Int within a Map, List or Tuple2") {
    it ("should serialize properly") {
      ...
      serializer.in[MyJsonObject](serializer.out(a)) must equal(a)
      val obj = serializer.in[MyJsonObject](serializer.out(a)) 
      obj must equal(a)
      obj.m.get("debasish").get.isInstanceOf[Int] must equal(true)
      obj.l.head.isInstanceOf[Int] must equal(true)
    }
  }
  ...
  describe("Testing Option for None values") {
    ...
    it("should serialize None to []") {
      new String(serializer.out(s)) must equal("""{"names":["dg","mc"],"user":[]}""")
    }

    it("should serialize and de-serialize correctly") {
      serializer.in[SampleConfigOption](serializer.out(s)) must equal(s)
    }
  }
  ...
  describe("Nested List serialization") {
    it("should give an instance of List of List of Int") {
      ...
      serializer.in[List[List[Int]]](serializer.out(l)) must equal(l)
    }
    it("should give an instance of List of List of String") {
      val l = List(List("a", "b"), List("c", "d", "e"), List("f"))
      serializer.in[List[List[String]]](serializer.out(l)) must equal(l)
    }
    it("should give an instance of List of List of Address") {
      ...
      serializer.in[List[List[Address]]](serializer.out(l)) must equal(l)
    }
  }
  ...
  describe("Serialization of Long in a bean") {
    it("should serialize") {
      ...
      serializer.in[Manager](serializer.out(m)) must equal(m)
    }
  }
...
/*
  describe("Serialization of optionally empty fields") {
    it("should serialize Family ignoring an empty children list") {
      ...
      new String(out) must equal("""{"father":{"age":40,"name":"Homer"},"mother":{"age":40,"name":"Marge"}}""")
      serializer.in[Family](out) must equal(pair)
    }
    ...
    it("should serialize a list of orphans without the empty parents in output") {
      ...
      new String(out) must equal("""{"children":[{"age":10,"name":"Bart"},{"age":8,"name":"Lisa"}]}""")
      serializer.in[Family](out) must equal(orphan)
    }
  }
*/
  ...
  describe("Serialization of classes containing singleton objects") {
    it("should serialize BondTrade with FI type of security") {
      ...
      serializer.in[BondTrade](out) must equal(fitrade)
    }
  ...
  describe("Serialization of classes containing collections of singleton objects") {
    it("should serialize an object with a Map containing lists of case objects") {
      ...
      serializer.in[TradedIn](out) must equal(t)
    }
  }
  ...
  describe("Serialization of classes containing Maps of Lists as values") {
    it("should serialize a Map of list of String") {
      ...
      val out = serializer.out(t)
      serializer.in[MapOfListOfString](out) must equal(t)
    }
    ...
    it("should serialize an optional Map of list of String") {
      ...
      val out = serializer.out(t)
      serializer.in[OptionalMapOfListOfString](out) must equal(t)
    }
    ...
    it("should serialize a Map of list of Shop") {
      ...
      val out = serializer.out(t)
      serializer.in[MapOfListOfShop](out) must equal(t)
    }
    ...
    it("should serialize an optional Map of list of Shop") {
      ...
      val out = serializer.out(t)
      serializer.in[OptionalMapOfListOfShop](out) must equal(t)
    }
    ...
    it("should serialize a Map of optional Shop") {
      ...
      val out = serializer.out(t)
      serializer.in[MapOfOptionalShop](out) must equal(t)
    }
    ...
    it("should serialize a List of Map of optional String") {
      ...
      val out = serializer.out(t)
      serializer.in[ListOfMapOfOptionalString](out) must equal(t)
    }
  }
  ...
  describe("String with $") {
    it ("should serialize with trailing $") {
      ...
      serializer.in[User](serializer.out(u)) must equal(u)
    }
