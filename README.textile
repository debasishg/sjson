...
<pre>val addr = Address("Market Street", "San Francisco", "956871")</pre>
...
<pre>addr should equal(
  serializer.in[Address](serializer.out(addr)))</pre>
...
<pre>serializer.in[AnyRef](serializer.out(addr))</pre>

or just as ..
<pre>serializer.in(serializer.out(addr))</pre>
...
<pre>val a = serializer.in[AnyRef](serializer.out(addr))
// use extractors
val c = 'city ? str
val c(ci) = a
...
<pre>val a1 = Address("Market Street", "San Francisco", "956871")
...
"Market Street" should equal(
  (r ># { ('addresses ? obj) andThen ('residence ? obj) andThen ('street ? str) }))</pre>
...
<pre>val b = new Book(100, "A Beautiful Mind", "012-456372")
...
<pre>it("should ignore issn field") {
...
<pre>val c = Contact("Bob", Map("residence" -> a1, "office" -> a2, "club" -> a3))
val co = serializer.out(c)
...
<pre>describe("Bean with optional bean member serialization") {
...
@BeanInfo case class D(@(JSONTypeHint @field)(value = classOf[A])param1: List[A])</pre>
and the serialization code like the following:

<pre>object TestSerialize{
...
<pre>val test1 = new D(List(B("hello1")))
